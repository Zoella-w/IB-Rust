...
//     //     _calculate_hash(&key1),
//     //     _calculate_hash(&key2)
//     // );
...
impl InventoryManager {
    ...
    fn add_book(&mut self, title: &str, author: &str, price: f64, quantity: u32) {
        ...
        // 添加到书名索引
        self.books_by_title
            .entry(title.to_lowercase()) // 获取小写书名的条目
            .or_insert_with(Vec::new) // 如果不存在，创建空 Vec
            .push(id); // 将当前 id 添加到对应书名的 Vec 中
    ...
    fn find_by_title(&self, title: &str) -> Vec<&Book> {
        ...
        self.books_by_title
            ...
            .map(|ids| {
                ids.iter()
                    .filter_map(|id| self.books_by_id.get(id)) // 将 ID 转换为 Book 引用（跳过无效 ID）
                    .collect()
            })
            .unwrap_or_else(Vec::new) // 处理 None 情况
    }
    ...
    fn update_quantity(&mut self, id: u32, delta: i32) -> Result<(), String> {
        if let Some(book) = self.books_by_id.get_mut(&id) {
            // 获取可变引用
            let new_quantity = book.quantity as i32 + delta; // 计算新库存​，使用 i32 处理负数变化
            // 返回错误信息防止负库存
            if new_quantity < 0 {
                return Err(format!(
    ...
    fn list_all_books(&self) {
        ...
        // 遍历所有书籍并打印，使用 Book 的 Display 实现
